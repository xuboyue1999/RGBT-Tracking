...
function [aveErrCoverageAll aveErrCenterAll] = calcPlotErrWJSR(seqs, trks,  bPlot)
basePath='..\dataset\';
...
%     seq_name = seq.name
    seq_name=seq;
    fileName = [basePath seq_name '/groundTruth_v.txt'];%groundTruth
    fileName
...
%         results.res=dlmread([basePath seq_name '\' name '_result.txt']); %seq_length*8
        results.res=dlmread([resultPath  seq_name '_' name  '.txt']); %seq_length*8
...
%         size((centerGT(1:seq_length,:)))
        err(:,index_algrm) = calcRectInt(rectMat(1:seq_length,:),rect_wh(1:seq_length,:));
%         rectMat(1,:)
%         rect_anno(1,:)
%         err(1,1)
...
%         errCenter(100,1)
        ...
        save([['results/' name] '/' name '_' seq_name '.mat'], 'err','errCenter');
        
        if bPlot            
            h1=figure(1);
%             plot(err(:,index_algrm),'color', trks{index_algrm}.color,'LineWidth',LineWidth,'LineStyle',LineStyle);
            plot(err(:,index_algrm),'color',[0,1,0],'LineWidth',LineWidth,'LineStyle',LineStyle);
            hold on 
            
            h2=figure(2);
            plot(errCenter(:,index_algrm),'color', [0,1,0],'LineWidth',LineWidth,'LineStyle',LineStyle);
            hold on
